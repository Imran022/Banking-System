...
Output files:
- Bank Account Transaction File (name given as command-line argument).

Run:
    python main.py <current_accounts_file> <transaction_output_file>

Example:
    python main.py currentaccounts.txt transout.atf
"""
...
class BankingConsole:
    ...
    def __init__(self, accounts_file: str, output_file: str) -> None:
        """Initialize controller with file paths from command-line arguments."""
    ...
    def load_accounts(self) -> bool:
        """Load the Current Bank Accounts file from the command-line path."""
        try:
            content = FileHandler.read_file(self.accounts_file)
...
def main() -> None:
    """Program entry point — expects two command-line arguments."""
    if len(sys.argv) != 3:
        print('Usage: python main.py <current_accounts_file> <transaction_output_file>')
        print('Example: python main.py currentaccounts.txt transout.atf')
        sys.exit(1)

    accounts_file = sys.argv[1]
    output_file = sys.argv[2]

    app = BankingConsole(accounts_file, output_file)
    app.run()
